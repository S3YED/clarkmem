"""
ClarkMem MCP server — exposes ingest/recall/stats as MCP tools so Claude Code,
Claude Desktop, or any MCP client can build and query a knowledge graph directly.

Run:  clarkmem-mcp                 (stdio transport)
Add to Claude Code:
      claude mcp add clarkmem -- clarkmem-mcp
Add to Claude Desktop (claude_desktop_config.json):
      {"mcpServers": {"clarkmem": {"command": "clarkmem-mcp"}}}

Backend + LLM are configured via env (CLARKMEM_BACKEND, ANTHROPIC_API_KEY or
...
try:
    ...
    raise SystemExit("MCP SDK not installed. Run: pip install 'clarkmem-kg[claude]'") from e

mcp = FastMCP("clarkmem")
_backend = None
...
@mcp.tool()
def clarkmem_ingest(text: str, tenant: str = "default", namespace: str = "default",
    ...
    r = clarkmem.ingest(_be(), text, tenant=tenant, namespace=namespace, is_path=is_path, key=key)
    return r.__dict__


@mcp.tool()
def clarkmem_recall(query: str, tenant: str = "default", k: int = 8,
    ...
    res = clarkmem.recall(_be(), query, tenant=tenant, namespace=namespace, k=k, hops=hops)
    return {
...
@mcp.tool()
def clarkmem_invalidate(subject: str, tenant: str = "default",
    ...
    n = clarkmem.invalidate(_be(), subject, tenant=tenant, predicate=predicate, object=object)
    return {"invalidated": n}


@mcp.tool()
def clarkmem_forget(doc_id: str, tenant: str = "default") -> dict:
...
@mcp.tool()
def clarkmem_stats(tenant: str = "default") -> dict:
