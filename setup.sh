#!/usr/bin/env bash
# One-command setup. Creates a venv, installs ClarkMem with the chosen backend
...
echo
echo "ClarkMem installed (backend extras: ${EXTRA})."
echo "  1. edit .env  -> set OPENROUTER_API_KEY (or your OpenAI-compatible key)"
echo "  2. source .venv/bin/activate && set -a && . ./.env && set +a"
echo "  3. clarkmem ingest examples/sample_docs/acme.md --tenant demo"
echo "     clarkmem recall 'what does Pathfinder run on?' --tenant demo"
